...
export type BirthdayEntry = { fullName: string; dob: string };

export async function fetchCurrentMonthBirthdays(): Promise<BirthdayEntry[]> {
  const sb = getSupabase();
  const { data, error } = await sb
    .from("contractor_profiles")
    .select("fullName, firstName, surname, dob")
    .eq("status", "Active")
    .neq("dob", "");
  ...
  return (data as { fullName: string; firstName: string; surname: string; dob: string }[])
