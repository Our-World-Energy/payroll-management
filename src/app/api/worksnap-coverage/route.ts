...
/**
 * Active contractors who exist in Contractor Details but have no record in
 * Worksnap at all — never a single time entry. Distinct from being absent on a
 * given day: these contractors aren't being tracked in the first place, so no
 * amount of waiting will produce time for them.
 *
 ...
 * worksnap_entries, not just one day's.
 */
...
export async function GET() {
  const [trackedEmails, profiles] = await Promise.all([
    prisma.worksnapEntry.findMany({ select: { email: true }, distinct: ["email"] }),
